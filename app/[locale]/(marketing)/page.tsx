            ...
            <section className="relative min-h-[90vh] flex items-center pt-24 pb-32 overflow-hidden bg-white">
                ...
                <div className="absolute top-0 right-0 w-1/2 h-full hidden lg:block pointer-events-none">
                    <div className="relative w-full h-full">
                        <VideoBackground
                            ...
                            overlayClassName="bg-gradient-to-t from-white via-transparent to-transparent opacity-90"
                        />
                        {/* Soft white fade from left to blend with text area */}
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-20 w-full">
                    <div className="lg:w-3/5">
                        ...
                        {/* SEARCH BAR - Full Width Long Design */}
                        <div className="w-full bg-white p-3 rounded-[3rem] shadow-[0_30px_80px_-20px_rgba(0,0,0,0.1)] border border-slate-50 flex flex-col lg:flex-row gap-2 animate-in fade-in slide-in-from-bottom-10 duration-1000 relative">
                            <Link href={`/${locale}/doctors`} className="flex-1 flex items-center gap-5 px-6 py-4 hover:bg-slate-50 transition-all rounded-[2rem] group border-r border-slate-50 lg:border-r">
                                <div className="w-12 h-12 rounded-2xl bg-cyan-50 flex items-center justify-center text-cyan-500 group-hover:bg-cyan-100 transition-colors shrink-0">
                                    <span className="material-symbols-outlined font-light">location_on</span>
                                </div>
                                <div className="flex flex-col text-left min-w-0">
                                    <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest mb-0.5 whitespace-nowrap">{isRu ? 'Расположение' : 'ÜNVAN'}</span>
                                    <span className="text-[15px] font-bold text-slate-900 truncate">{isRu ? 'Выберите Город' : 'Şəhər Seçin'}</span>
                                </div>
                            </Link>
                            ...
                            <Link href={`/${locale}/doctors`} className="flex-1 flex items-center gap-5 px-6 py-4 hover:bg-slate-50 transition-all rounded-[2rem] group border-r border-slate-50 lg:border-r">
                                ...
                                <div className="flex flex-col text-left min-w-0">
                                    <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest mb-0.5 whitespace-nowrap">{isRu ? 'Специальность' : 'İXTİSAS'}</span>
                                    <span className="text-[15px] font-bold text-slate-900 truncate">{isRu ? 'Выберите Услугу' : 'Xidmət Seçin'}</span>
                                </div>
                            </Link>

                            <Link href={`/${locale}/doctors`} className="flex-1 flex items-center gap-5 px-6 py-4 hover:bg-slate-50 transition-all rounded-[2rem] group">
                                <div className="w-12 h-12 rounded-2xl bg-cyan-50 flex items-center justify-center text-cyan-500 group-hover:bg-cyan-100 transition-colors shrink-0">
                                    <span className="material-symbols-outlined font-light">calendar_today</span>
                                </div>
                                <div className="flex flex-col text-left min-w-0">
                                    <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest mb-0.5 whitespace-nowrap">{isRu ? 'Дата' : 'TARİX'}</span>
                                    <span className="text-[15px] font-bold text-slate-900 truncate">{isRu ? 'Выберите Дату' : 'Tarix Seçin'}</span>
                                </div>
                            </Link>
                            ...
                            <div className="flex items-center p-1">
                                <Link
                                    href={`/${locale}/doctors`}
                                    className="h-[64px] min-w-[200px] px-8 rounded-full bg-cyan-500 hover:bg-cyan-400 text-white font-black transition-all flex items-center justify-center gap-3 whitespace-nowrap active:scale-95 shadow-xl shadow-cyan-100 hover:shadow-cyan-200"
                                >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        {[
                            ...
                            <div key={idx} className="group relative p-8 rounded-[2.5rem] bg-white border border-slate-100 hover:border-cyan-200 transition-all duration-500 hover:shadow-2xl flex flex-col items-center text-center">
                                <div className="w-full aspect-video mb-8 relative rounded-2xl overflow-hidden shadow-lg group-hover:shadow-xl transition-all">
                                    <Image
                                ...
                                <div className="size-14 rounded-2xl bg-white text-cyan-600 shadow-md flex items-center justify-center mb-6 group-hover:bg-cyan-500 group-hover:text-white transition-all transform group-hover:-translate-y-2 border border-slate-50">
            ...
            <section className="py-24 bg-slate-50">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6">
                        ...
                        <Link href={`/${locale}/doctors`} className="bg-white px-8 py-4 rounded-2xl border border-slate-200 shadow-sm font-black text-[12px] uppercase tracking-widest hover:bg-slate-900 hover:text-white transition-all hover:shadow-xl hover:-translate-y-1">
                            {t('Common.view_all')}
                        </Link>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                        {doctors.map((doc, idx) => (
                            <div key={idx} className="bg-white rounded-[2rem] overflow-hidden border border-slate-100 shadow-xl shadow-slate-200/50 hover:-translate-y-2 transition-all duration-500 group relative">
                                <div className="aspect-[3/4] relative overflow-hidden">
                                    ...
                                    <div className="absolute top-4 right-4 bg-white/30 backdrop-blur-md px-3 py-1.5 rounded-xl flex items-center gap-1.5 border border-white/50 text-white font-bold">
                                        <span className="material-symbols-outlined text-amber-300 text-[16px] fill-current">star</span>
                                        <span className="text-sm">{doc.rating}</span>
                                    </div>
                                </div>
                                <div className="p-6 relative">
                                    <div className="absolute -top-10 right-6 size-14 rounded-2xl bg-white shadow-lg flex items-center justify-center text-cyan-600 group-hover:bg-cyan-500 group-hover:text-white transition-all transform group-hover:rotate-12">
                                        <span className="material-symbols-outlined text-[28px]">{doc.icon}</span>
                                    </div>
                                    <div className="mb-4">
                                        <span className={`inline-block px-3 py-1 rounded-lg ${doc.bgColor} ${doc.color} text-[10px] font-black uppercase tracking-wider mb-2`}>
                                            {doc.spec}
                                        </span>
                                        <h3 className="text-lg font-black text-slate-900 leading-tight group-hover:text-cyan-600 transition-colors">{doc.name}</h3>
                                        <p className="text-xs text-slate-400 font-bold mt-1">{doc.location}</p>
                                    </div>
                                    <div className="border-t border-slate-100 pt-4 flex items-center justify-between">
                                        <div className="text-xs font-bold text-slate-400">
                                            {doc.reviews} {isRu ? 'отзывов' : 'rəy'}
                                        </div>
                                        <Link
                                            href={`/${locale}/doctors`}
                                            className="size-10 rounded-xl bg-slate-50 flex items-center justify-center text-slate-900 group-hover:bg-slate-900 group-hover:text-white transition-colors"
                                        >
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                        {specialties.map((cat, idx) => (
                            <Link
                                ...
                                className="group p-8 rounded-[2rem] border border-slate-100 hover:border-cyan-200 hover:shadow-2xl transition-all duration-500 flex flex-col items-center text-center relative overflow-hidden"
                            >
                                <div className={`size-20 rounded-[1.5rem] bg-${cat.color}-50 text-${cat.color}-600 flex items-center justify-center mb-6 group-hover:scale-110 transition-all transform shadow-sm group-hover:shadow-md`}>
                                    <span className="material-symbols-outlined text-[36px]">{cat.icon}</span>
                                </div>
                                <h3 className="font-black text-slate-900 text-lg group-hover:text-cyan-600 transition-colors">{cat.name}</h3>
                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-2">{cat.count} {isRu ? 'врачей' : 'həkim'}</p>

                                <div className="absolute inset-0 bg-gradient-to-t from-white/0 via-white/0 to-white/0 group-hover:from-cyan-50/30 transition-all pointer-events-none"></div>
                            </Link>
                        ))}
                    </div>
                </div>
